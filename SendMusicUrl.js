...
function doPost(e) {
  ...
  const description = {
    song:  `/ ${spotifyInfo.artists} + ${spotifyInfo.album_name || ''}`,
    album: `/ ${spotifyInfo.artists} (${info.album_type})`,
    artist:`(${spotifyInfo.genres})`
  };
  const info = `
  ${spotifyInfo.name} ${description[spotifyInfo.type]}
  :spotify:    ${spotifyInfo.link} 
  :applemusic: ${appleMusicInfo.link}
  `.replace(/^\n|\s+$|^ {2}/gm, '');  // 2: indent spaces of formated code
...
function searchInSpotify(queryTextsCand) {
  ...
  switch (type) {
    ...
    case 'album':
      info.artists = item.artists.map(artist => artist.name).join(', ');
      info.album_type = item.album.album_type;
    case 'artist':
